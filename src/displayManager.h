...
public:
    ...
    void loop() {
        ...
        if (timeDifference > 15000 && targetBrightness != 0) {
            targetBrightness = 0;
            // display.ssd1306_command(SSD1306_SETPRECHARGE);
    ...
    void drawSelection(const char* lineBuffer) {
        ...
        u8g2.setDrawColor(1);
        int len = (256 - strlen(lineBuffer) * 7) / 2; // Adjusted for font width
        u8g2.drawStr(len, 35, lineBuffer);
    }
    ...
    void drawSlider(float currentValue,int maxMap, int minVal, int maxVal) {
            ...
            int xPos = (int)(20 + (currentValue / (float)maxMap) * 195.0f);
            int progress = (int)percentage;
            int ySlider = 29;
            int yText = 32;

            u8g2.setFont(u8g2_font_6x10_tf);
            u8g2.drawLine(20, ySlider, 215, ySlider); // horizontal line
