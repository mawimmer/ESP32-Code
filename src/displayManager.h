...
public:
    ...
    void drawSelection(const char* enumName) {
        //println(title)
        display.setTextSize(1);
        display.setTextColor(SSD1306_WHITE);
        display.setCursor(0,0);
        display.println(enumName);
    }

    void drawSlider(const char* sliderName, int currentValue, int minVal, int maxVal) {
        //draw symbol
        //draw title
    ...
    {
        display.clearDisplay();
        drawSlider(sliderName, currentValue, minVal, maxVal);
        drawNavHint(leftHint, rightHint, turnHint);
        display.display();
    }

    void renderSelectionScreen(const char* enumName, const char* leftHint, 
                            const char* rightHint, const char* turnHint)
    {
        display.clearDisplay();
        drawSelection(enumName);
        drawNavHint(leftHint, rightHint, turnHint);
        display.display();
    }
